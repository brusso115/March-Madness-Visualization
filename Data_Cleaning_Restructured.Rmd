---
title: "Data_Cleaning_Restructured"
author: "Bailey Russo"
date: "4/16/2021"
...
# Predicting Score Between Two Teams - Method 1
```{r}
...
```
# Calculating Strength of conference
...
# Linear Regressions To Predict Game Score
```{r}
...
```{r}
SOS = test %>% group_by(Season,TeamID,TeamName) %>% summarise(SOS = mean(OppSeasonAdjusted_EM)) %>% arrange(desc(Season),desc(SOS))
SOS = inner_join(SOS, TeamConferences, by=c("Season","TeamID"))
SOS = inner_join(SOS,test_conf,by=c("Season","ConfAbbrev"))
#fileName = paste(path_out, 'SOS.csv',sep = '')
#write.csv(SOS,fileName)
```

```{r}
SOS %>% group_by(Season,ConfAbbrev) %>% summarise(ConfStrength = mean(Adjusted_EM_Conf)) %>% arrange(desc(Season),desc(ConfStrength))
#fileName = paste(path_out, 'conf_strength.csv',sep = '')
#write.csv(test_conf,fileName)
```
```{r}
colnames(test_HCA)
